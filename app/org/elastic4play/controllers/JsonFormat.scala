...
object JsonFormat {
  ...
  private val fileReads = Reads[File] { json ⇒ json.validate[String].map(filepath ⇒ new File(filepath)) }
  ...
  private val pathReads = Reads[Path] { json ⇒ json.validate[String].map(filepath ⇒ Paths.get(filepath)) }
  ...
  private val fileInputValueWrites = Writes[FileInputValue] { (fiv: FileInputValue) ⇒ fiv.jsonValue + ("type" → JsString("FileInputValue")) }
  private val stringInputValueReads = Reads[StringInputValue] { json ⇒ (json \ "value").validate[Seq[String]].map(s ⇒ StringInputValue(s)) }
  private val jsonInputValueReads = Reads[JsonInputValue] { json ⇒ (json \ "value").validate[JsValue].map(v ⇒ JsonInputValue(v)) }
  ...
  private val inputValueWrites = Writes[InputValue]((value: InputValue) ⇒ value match {
