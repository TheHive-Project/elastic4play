...
object JsonFormat {
  implicit def baseModelEntityWrites[E <: BaseEntity]: Writes[E] = Writes((entity: E) => entity.toJson)

  implicit def multiFormat[T](implicit jsFormat: Format[T]): Format[Seq[T]] = Format(Reads.seq(jsFormat), Writes.seq(jsFormat))

  private def optionReads[T](implicit jsReads: Reads[T]): Reads[Option[T]] = Reads[Option[T]] {
