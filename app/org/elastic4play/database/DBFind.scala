...
class DBFind(pageSize: Int, keepAlive: FiniteDuration, db: DBConfiguration, implicit val ec: ExecutionContext, implicit val mat: Materializer) {
  ...
  /**
    ...
    */
  private[database] def searchWithScroll(
      searchDefinition: SearchDefinition,
      offset: Int,
      limit: Int
  ): (Source[RichSearchHit, NotUsed], Future[Long]) = {
  ...
  /**
    ...
    */
  private[database] def searchWithoutScroll(
      searchDefinition: SearchDefinition,
      offset: Int,
      limit: Int
  ): (Source[RichSearchHit, NotUsed], Future[Long]) = {
  ...
  def apply(range: Option[String], sortBy: Seq[String])(query: (String) ⇒ SearchDefinition): (Source[JsObject, NotUsed], Future[Long]) = {
    ...
    logger.debug(
      s"search in ${searchDefinition.indexesTypes.indexes.mkString(",")} / ${searchDefinition.indexesTypes.types.mkString(",")} ${SearchBuilderFn(db.client.java, searchDefinition)}"
    )
...
class SearchWithScroll(db: DBConfiguration, searchDefinition: SearchDefinition, keepAliveStr: String, offset: Int, max: Int)(
    implicit ec: ExecutionContext
) extends GraphStage[SourceShape[RichSearchHit]] {
